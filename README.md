...
Penguins of Tangram is designed for children ages 4–10: Arrow keys or WASD move, Space/Up jumps, and P or Escape pauses. Enemy hits suspend movement until the respawn animation safely returns the player to the latest checkpoint. The game starts in Dutch and the in-game How to play panel can switch to English; the choice is saved on the device. The same panel explains keyboard and touch controls and includes settings for sound, reduced motion, and the default-on touch-control circles, plus route notes and the parent-confirmed reset action. Progress, control preferences, and optional route notes stay on the device; no account, network service, advertising, chat, or tracking is required.
